    ...
    async def _woke_main_agent(
        ...
        *,
        ...
        if context:
            ...
            req.contexts = []
            history_wrap_prompt = self.ctx.get_config().get("cron_history_wrap_prompt", "")
            try:
                req.system_prompt += (
                    history_wrap_prompt.format(context_dump=context_dump)
                )
            except Exception:
                logger.error("cron_history_wrap_prompt 文案格式化失败，已退回默认格式", exc_info=True)
                req.system_prompt += (
                    CONVERSATION_HISTORY_INJECT_PREFIX.format(context_dump=context_dump)
                )
        ...
        try:
            ...
            logger.error("cron_execution_prompt 文案格式化失败，已退回默认格式", exc_info=True)
            req.system_prompt += PROACTIVE_AGENT_CRON_WOKE_SYSTEM_PROMPT.format(
                cron_job=cron_job_str
            )
        req.prompt = (self.ctx.get_config().get("cron_task_work_user_prompt", "")
        ...
        try:
            ...
            logger.error("cron_task_summary_note 文案格式化失败，已退回默认格式", exc_info=True)
            summary_note = (
        ...
        if llm_resp and llm_resp.role == "assistant":
            result_prompt = self.ctx.get_config().get("cron_task_summary_note_result", "")
            try:
                result_prompt = result_prompt.format(result=llm_resp.completion_text)
            except Exception:
                logger.error("cron_task_summary_note_result 文案格式化失败，已退回默认格式", exc_info=True)
                summary_note += (
